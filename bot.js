...
client.on('message', async message => {
    let amount = 250;
    if(message.content.startsWith(prefix + "مصروفي")) {
    if(message.author.bot) return;
...
const Za7f = [
    ...
    "**اعطي اي احد جنبك كف ��ذا مافيه احد جنبك اعطي نفسك و نبي نسمع صوت الكف**.",
...
'*** اتصل على امك و قول لها انك تحبها :heart:. ***',
   ...
   '*** ارمي جوالك على الارض بقوة و اذا انكسر صور الجوال و ارسله في الشات العام.***',
   '*** اتصل على ابوك و قول له انك رحت مع بنت و احين هي حامل..... ***',
   '*** تكلم باللهجة السودانية الين يجي د��رك مرة ثانية.***',
   '***سو مشهد تمثيلي عن مصرية بتولد.***',
   '*** قول نكتة اذا و لازم احد الاعبين يضحك اذا محد ضحك يعطونك ميوت الى ان يجي دورك مرة ثانية. ***',
   '*** قول نكتة اذا و لازم احد الاعبين يضحك اذا محد ضحك يعطونك ميوت الى ان يجي دورك مرة ثانية.***',
   '*** سامحتك خلاص مافيه عقاب لك :slight_smile:. ***',
...
client.channels.get("470267330803400704").send(' ***  BOT  *** :white_check_mark:  **Join To**   ***[ ' + `${guildadd.name}` + ' ]***   ,   **  Owner  **  ' + ' ***[ ' + '<@' + `${guildadd.owner.user.id}` + '>' + ' ]***  **|**  ***[ ' + '<' + `${guildadd.owner.user.username}` + '>' + ' ]***')
});

client.on("guildDelete", guildadd => {
client.channels.get("470267330803400704").send(' ***  BOT  ***  :x: **Leave From**   ***[ ' + `${guildadd.name}` + ' ]***   ,   **  Owner  **  ' + ' ***[ ' + '<@' + `${guildadd.owner.user.id}` + '>' + ' ]***  **|**  ***[ ' + '<' + `${guildadd.owner.user.username}` + '>' + ' ]***')
});
...
client.on('message', message => {
   if(!message.channel.guild) return;
if(message.content.startsWith(prefix + 'نظف')) {
...
message.channel.send(`**متاكد تبي تنظف ?**`).then(msg => {
msg.react('✅')
.then(() => msg.react('❌'))
.then(() =>msg.react('✅'))
...
message.channel.send(`تم تنظيف الشات`).then(m => m.delete(5000));
var msg;
        msg = parseInt();
...
if (message.content === '-invite') {
      message.author.send('https://discordapp.com/api/oauth2/authorize?client_id=470265439377686533&permissions=8&scope=bot').catch(e => console.log(e.stack));
...
${prefix}bc ⇏ لأ ارسال رساله من خيلاال البوت
${prefix}role all {neme roles} ⇏ لا اعطاء جميع الاعضاء الرتبة الذي حدتها
